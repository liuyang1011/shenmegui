...
<form class="easyui-form" id="exportForm">
    ...
    <div id="mm" class="easyui-menu" style="width:120px;">
        <div onclick="chooseUnStandard()" data-options="iconCls:'icon-edit'">设置详细</div>
    </div>
    ...
    <table id="choosedList" class="easyui-datagrid"
        ...
        <tr>
            ...
            <th data-options="field:'versionId',width:100,editor:versionEditor" rowspan="2">服务版本</th>
            <th data-options="field:'versionAutoId',hidden:true">版本Id</th>
            <th data-options="field:'proTranCode',hidden:true">交易码</th>
        ...
        <tr>
            ...
            <th data-options="field:'proGeneratorName',width:100,editor:proGeneratorEditor">生成模式</th>
    </table>
</form>
<script type="text/javascript">
    ...
    var versionEditor = {
        ...
        options: {
            ...
            textField: 'versionId',
            method: 'GET',
            url: "/operationHis/getVersionsByOS/" + serviceId + "/" + operationId,
            onSelect: function (record) {
                var row = $("#choosedList").datagrid("getSelected");
                var index = $("#choosedList").datagrid("getRowIndex", row);
                $("#choosedList").datagrid("endEdit", index);
                ...
                $("#choosedList").datagrid("updateRow", {
                    index: index,
                    row: {
                        versionAutoId: record.versionCode
                    }
                });
    ...
    function exportBath() {
        ...
        if (null != rows && 0 < rows.length) {
            ...
            var fields = ["consumerServiceInvokeId", "providerServiceInvokeId", "conGeneratorId", "proGeneratorId", "versionAutoId", "versionId", "proGeneratorName", "conGeneratorName"];
    ...
    function tranToEsb() {
        ...
        if (null != rows && 0 < rows.length) {
            ...
            for (var i = 0; i < rows.length; i++) {
                ...
                configVO.conGeneratorName = rows[i].conGeneratorName;
                configVO.proGeneratorName = rows[i].proGeneratorName;
                configVO.providerName = rows[i].providerName;
                configVO.versionId = rows[i].versionId;
                configVO.versionAutoId = rows[i].versionAutoId;
                configVO.serviceId = serviceId;
                configVO.operationId = operationId;
                configVO.proInterfaceName = rows[i].proInterfaceName;
                configVO.proTranCode = rows[i].proTranCode;
