 ...
 */
@Component
public class CoreXMLConfigExportGender extends ConfigExportGenerator {
    private Log log = LogFactory.getLog(CoreXMLConfigExportGender.class);
    ...
    public void generateRequest(ServiceInvoke serviceInvoke, String path) {
        try {
            ...
            if (null != inter) {
                ...
                Element appElement = serviceElement.addElement("app");
                ...
                fillContent(appElement, children, serviceId, operationId);
    ...
    public void fillContent(Element element, List<Ida> idas) {
        for (Ida ida : idas) {
            Element idaElement = fillContentTag(element, ida);
            List<Ida> children = idaService.getNotEmptyByParentId(ida.getId());
            fillContent(idaElement, children);
        }
    }

    public Element fillContentTag(Element element, Ida ida) {
        Element idaElement = element.addElement(ida.getStructName());
        addAttribute(idaElement, "metadataid", ida.getMetadataId());
